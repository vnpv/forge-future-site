...
export default function HomeForgeGem() {
  ...
  return (
    <section
      id="forge-gem"
      className="scroll-mt-24 border-2 border-white/[0.08] bg-[#0c0c0c] p-6 sm:p-8"
    >
      <p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-[#e8951a]">
        {section.tag}
      </p>
      <h2 className="ff-display mb-3 text-2xl font-extrabold text-white sm:text-3xl">
        {section.h2}
      </h2>
      ...
      <ol className="grid gap-4 sm:grid-cols-2">
        {forgeGem.stages.map((stage, i) => (
          <li key={stage.key}>
            <Link
              href={stage.href}
              className="ff-explore-card group flex h-full flex-col border-2 border-white/[0.08] bg-[#141414] p-5"
            >
              <span className="ff-display mb-2 text-lg font-extrabold text-[#e8951a]/70">
                {String(i + 1).padStart(2, "0")}
              </span>
              <h3 className="ff-display text-lg font-bold text-white group-hover:text-[#e8951a] transition-colors">
                {stage.title}
              </h3>
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-[#6b7280]">
                {stage.subtitle}
              </p>
              <p className="flex-1 text-sm leading-relaxed text-[#8b9199]">
                {stage.desc}
              </p>
            </Link>
          </li>
